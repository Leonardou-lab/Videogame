...
2. Game Design
   ...
   - 2.5 Visual Style
   - 2.6 References
   - 2.7 Visual Evolution: Planned vs. Final
3. Technical
   - 3.1 Screens
   - 3.2 Controls
   - 3.3 Mechanics
...
10. Repository Organization and Installation
...
| Into the Breach — tactical board reference | Slay the Spire — card and roguelite reference | Dungeon of the Endless — survival reference |
| :---: | :---: | :---: |
| ![Into the Breach reference](Videogame/Assets/images/reference-into-the-breach.png) | ![Slay the Spire reference](Videogame/Assets/images/reference-slay-the-spire.png) | ![Dungeon of the Endless reference](Videogame/Assets/images/reference-dungeon-of-the-endless.png) |

## 2.7 Visual Evolution: Planned vs. Final

The visual design evolved through sketches, generated concepts, early prototypes, and implementation passes. The following comparisons document both what the team originally expected and what was ultimately delivered in the playable project.

### Planned Card Language

The early card-system concept organized attack, defense, and political cards into clearly separated families. It established the medieval pixel-art direction, visible AP costs, combat statistics, tactical diagrams, and the idea that each card should read as a collectible game object rather than a generic interface button.

![Planned card system and visual language](Videogame/Assets/images/planned-card-system.png)

The final game retains this foundation through individual card artwork, visible AP values, card statistics, hover descriptions, upgrade variants, and the complete 15-card gallery documented earlier in this GDD. Royal Decree was later removed from the playable pool and replaced by Bomb.

### Early Board Prototype

The first visual board prototype focused only on validating the essential idea: an 8×8 board, a King at the center, a moving 3×3 safe zone, enemies approaching from the edge, and allied protection around the King.

![Early board prototype](Videogame/Assets/images/early-board-prototype.png)

This simple prototype proved the board and safe-zone concept before the team invested in sprites, card artwork, level backgrounds, Desperation, bosses, statistics, audio, and backend integration.

### Main Menu — Expected Appearance

The planned menu established the throne-room background, centered logo, gold-and-purple palette, overview panel, and large medieval buttons.

![Planned main menu](Videogame/Assets/images/planned-main-menu.png)

### Main Menu — Final Implementation

The final menu keeps the same throne-room identity while functioning as the central hub for login, Start Game, Tutorial, Statistics, Credits, and Settings. It also integrates the authenticated username and internal views without redirecting to separate website pages.

![Final implemented main menu](Videogame/Assets/images/final-main-menu.png)

### Gameplay Screen — Expected Appearance

The planned gameplay mockup imagined a dense medieval HUD with the board at the center, a full card row, upgrades, King information, enemy formations, AP, gold, and horde progress.

![Planned gameplay screen](Videogame/Assets/images/planned-gameplay-screen.png)

### Gameplay Screen — Final Implementation

The implemented screen preserves the central board and side-panel strategy while simplifying the information hierarchy. Cards remain visible on the left, the board occupies the center, and level data, Desperation, King movement, turn controls, and upgrades remain on the right. This layout keeps the current level background visible across the complete page.

![Final implemented gameplay screen](Videogame/Assets/images/final-gameplay.png)

### Statistics — Final Implementation

The final Statistics view goes beyond the initial mockups. It reads personal data from the API and database, displays total runs, kills, gold, and completed levels, and visualizes run performance and card upgrades with charts. Administrator accounts can access additional global analytics and rankings.

![Final implemented statistics screen](Videogame/Assets/images/final-statistics.png)
...
This section merges the repository guide with the final GDD so instructors can understand, install, run, and evaluate the complete project from one document.
...
├── TheCowardKing-GDD-Updated.md      # Implementation-aligned Markdown GDD
├── TheCowardKing-GDD-FINAL.md        # Final unified Markdown document
├── TheCowardKing-GDD-FINAL.docx      # Final unified Word document
├── .gitignore
├── .github/
...
└── Videogame/                        # Current playable game, frontend, backend, and assets
    ...
    └── Assets/
        ...
        ├── images/                   # Menu, logo, screenshots, diagrams, and Desperation images
        ├── sprites/                  # King, ally, enemy, defense, and boss sprites
        └── videos/tutorial.mp4       # Tutorial video embedded in the menu
```

The files inside `activities/` preserve coursework, experiments, and earlier deliveries. They provide academic evidence and development history but are not required to run the current game. The executable project is located inside the second `Videogame/` directory.

`Database/node_modules/` is generated by `npm install`; it is not project source code and should remain ignored by Git.

## 10.2 Main Entry Points

- Open `Videogame/indexMenu.html` through a local web server to enter the main menu.
- **Start Game** opens `Videogame/index.html`.
- The frontend expects the REST API at `http://localhost:3000`.
- Without the API, the game can use an 8-card fallback pool. Login, SQL statistics, persistent database upgrades, and the full 15-card pool require MySQL and the backend.
...
> **Warning:** The script begins with `DROP DATABASE IF EXISTS coward_king`. Running it again removes and recreates the existing local database.

### Step 3 — Configure MySQL credentials

Open `Videogame/Database/server.js` and enter the credentials of the local MySQL installation:
...
The MySQL credentials are different from the player accounts created inside the game.

### Step 4 — Install and run the backend API
...
Keep the terminal open. A successful startup displays:

```text
API running on http://localhost:3000
```

Open `http://localhost:3000/api/cards` to verify that the API returns the card list as JSON.
...
Opening the HTML directly through a `file://` address is not recommended because browser restrictions can interfere with fetch requests, audio, and local assets.

### Step 6 — Create a player account

1. Select **Login** from the main menu.
2. Enter a username and password.
3. Select **Sign Up**.
4. Log in with the same player account.

## 10.4 Verification Checklist

- The menu loads from `indexMenu.html`.
- Music starts after the first browser interaction.
- **Start Game** opens the 8×8 board.
- `/api/cards` returns 15 cards when MySQL and the API are active.
- Login creates or retrieves a player.
- Personal statistics and purchased upgrades load for a logged-in player.
- Level backgrounds, sprites, cards, music, videos, and sound effects load without missing-file errors.
...
- **Cards endpoint error:** Inspect the API terminal and confirm that MySQL Server is active.
- **Player not found:** Use **Sign Up** before logging in.
- **Music is initially silent:** Interact with the page once because browsers block automatic audio playback.
- **Only 8 cards appear:** The local fallback pool is active because the API or database did not load.

## 10.6 Current Save Limitation

The pause-menu button stores a partial snapshot in browser `localStorage`, while the API records users, runs, statistics, and upgrades. Complete reconstruction of an interrupted board state is not implemented; therefore, the current save must not be described as a complete Resume Game system.

---

*Final unified document prepared by Silent Crown — The Coward King.*
